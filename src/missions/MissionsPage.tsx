...
import { getAllMissions, clearAllMissions, MissionRecord } from '../utils/storage';
...
const MissionsPage: React.FC = () => {
	...
	}, [missions, searchQuery, showCompleted, showUncompleted, difficultyFilter, sortConfig]);
	...
	const handleExport = () => {
		...
		link.href = url;
		link.download = `autosupper-missions-${new Date().toISOString().split('T')[0]}.json`;
		link.click();
	...
	const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		reader.onload = async (e) => {
			try {
				...
				alert(`Imported ${importedCount} missions (${skippedCount} duplicates skipped)`);
				loadMissions();
			} catch (error) {
	...
	return (
		<div className="missions-page">
			<header className="missions-header">
				<h1>LazyFrog Mission Manager</h1>
				...
				<table className="missions-table">
					...
					<tbody>
						{filteredMissions.map((mission) => (
							<tr key={mission.postId} className={mission.cleared ? 'completed' : ''}>
								...
								<td className="mission-cell">
									...
									{mission.tags && <div className="mission-tags">{mission.tags}</div>}
								</td>
								<td>{mission.username}</td>
								<td className="difficulty-cell">{renderStars(mission.difficulty)}</td>
									...
									{mission.totalLoot && mission.totalLoot.length > 0 ? (
										<span
											title={mission.totalLoot
												.map((item) => `${item.quantity}× ${formatItemName(item.id)}`)
												.join(', ')}
										...
										<span title="No rewards captured" style={{ opacity: 0.3 }}>
											—
										</span>
									)}
								</td>
