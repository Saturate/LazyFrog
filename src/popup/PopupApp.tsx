...
const PopupApp: React.FC = () => {
    ...
    missions.forEach((mission) => {
      ...
      mission.metadata.mission.encounters.forEach((encounter: any) => {
        if (encounter.type === "abilityChoice") {
          if (encounter.optionA?.abilityId)
            abilitiesSet.add(encounter.optionA.abilityId);
          if (encounter.optionB?.abilityId)
            abilitiesSet.add(encounter.optionB.abilityId);
          if (encounter.optionC?.abilityId)
            abilitiesSet.add(encounter.optionC.abilityId);
        }
      });
    });
    ...
    missions.forEach((mission) => {
      ...
      mission.metadata.mission.encounters.forEach((encounter: any) => {
        if (encounter.type === "skillBargain") {
          if (encounter.positiveEffect?.stat)
            statsSet.add(encounter.positiveEffect.stat);
          if (encounter.negativeEffect?.stat)
            statsSet.add(encounter.negativeEffect.stat);
        }
  ...
  const loadMissions = async () => {
    try {
      ...
      const missionList = Object.values(allMissions).sort(
      ...
      setAutomationConfig((prev) => {
        const newAbilities = discoveredAbilities.filter(
          (a) => !prev.abilityTierList.includes(a)
        );
        const newStats = discoveredStats.filter(
          (s) => !prev.blessingStatPriority.includes(s)
        );
        ...
        if (newAbilities.length > 0) {
          ...
          console.log(
            "[POPUP] Total abilities now:",
            discoveredAbilities.length
          );
        }
    ...
    chrome.storage.local.get(["filters", "automationConfig"], (result) => {
      ...
      if (result.automationConfig) {
        ...
        setAutomationConfig((prev) => ({
          ...
          abilityTierList:
            result.automationConfig.abilityTierList || prev.abilityTierList,
          blessingStatPriority:
            result.automationConfig.blessingStatPriority ||
            prev.blessingStatPriority,
        }));
      }
    });
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        if (chrome.runtime.lastError) {
          ...
          console.log(
            "[POPUP] ✅",
            response.message || "Navigating to next mission"
          );
        }
      }
    );
    ...
    <div className="popup-container">
      <header className="popup-header">
        ...
        <p className="author">
          Made by{" "}
          <a
            href="https://www.reddit.com/user/AKJ90"
            target="_blank"
      ...
      {currentTab === "control" && (
        <>
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              <div className="form-group" style={{ flex: 1 }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateFilter("minLevel", parseInt(e.target.value) || 1)
                  }
                  disabled={isRunning}
                  min="1"
                />
              ...
              <div className="form-group" style={{ flex: 1 }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateFilter("maxLevel", parseInt(e.target.value) || 340)
                  }
                  disabled={isRunning}
                  min="1"
                />
                ...
                {(() => {
                  const matchingMissions = missions.filter((m) => {
                    ...
                    if (!filters.stars.includes(m.difficulty || 0))
                      return false;
                    if (m.minLevel && m.minLevel < filters.minLevel)
                      return false;
                    if (m.maxLevel && m.maxLevel > filters.maxLevel)
                      return false;
                    return true;
                  });
              ...
              <div className="results-content">
                {levels.length === 0 ? (
                  ...
                  <>
                    ...
                    {levels.length > 5 && (
                      <p className="more">...and {levels.length - 5} more</p>
                    )}
                  </>
                )}
              </div>
          ...
          <div className="section">
            ...
            {missions.length === 0 ? (
              <p className="help-text">
                No missions scanned yet. Play some missions to populate the
                database!
              </p>
            ) : filteredMissions.length === 0 ? (
              <p className="help-text">
                No uncleared missions match your current filters. Try adjusting
                star difficulty or level range.
              </p>
            ) : (
      ...
      {currentTab === "missions" && (
        <>
          ...
          <div className="section">
            ...
            <p className="help-text">
              Scanner runs automatically as you scroll Reddit. All missions are
      ...
      {currentTab === "options" && (
        <>
          <div className="section">
            ...
            <p className="help-text">
              Drag to reorder. Abilities will be selected in this order of
              preference. All abilities discovered from mission metadata are
              shown below:
            </p>
            {automationConfig.abilityTierList.length === 0 ? (
              ...
              >
                No abilities discovered yet. Play missions or scan Reddit to
                discover abilities!
              </p>
            ) : (
          ...
          <div className="section">
            ...
            <p className="help-text">
              Drag to reorder. Stats will be prioritized in this order when
              choosing blessings. All stats discovered from mission metadata are
              shown below:
            </p>
            {automationConfig.blessingStatPriority.length === 0 ? (
              ...
              >
                No blessing stats discovered yet. Play missions or scan Reddit
                to discover stats!
              </p>
            ) : (
              <div className="tier-list">
                {automationConfig.blessingStatPriority.map((stat, index) => (
                  <div
                    ...
                    onDrop={(e) => {
                      ...
                      const newPriority = [
                        ...automationConfig.blessingStatPriority,
                      ];
          ...
          <div className="section">
            ...
            <div className="form-group">
              <label htmlFor="skillBargainStrategy">
                When to accept skill bargains:
              </label>
              <select
                id="skillBargainStrategy"
                value={automationConfig.skillBargainStrategy}
                onChange={(e) =>
                  updateAutomationConfig("skillBargainStrategy", e.target.value)
                }
              >
                <option value="always">Always Accept</option>
                <option value="positive-only">
                  Accept if Positive &gt; Negative
                </option>
            ...
            <div className="form-group checkbox">
              <label>
                <input
                  ...
                  onChange={(e) =>
                    updateAutomationConfig("debugMode", e.target.checked)
                  }
                />
                Enable Debug Mode
              </label>
            ...
            <div className="form-group checkbox" style={{ marginTop: "16px" }}>
              <label>
                <input
                  ...
                  onChange={(e) =>
                    updateAutomationConfig("remoteLogging", e.target.checked)
                  }
                />
              ...
              <button
                ...
                onClick={async () => {
                  if (
                    ...
                    const { clearAllMissions } = await import(
                      "../utils/storage"
                    );
