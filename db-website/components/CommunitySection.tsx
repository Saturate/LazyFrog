export function CommunitySection() {
  return (
    <div className="mt-12 max-w-4xl mx-auto">
      <div className="bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-900/20 dark:to-green-900/20 border-2 border-emerald-200 dark:border-emerald-800 rounded-xl p-8 text-center">
        <div className="text-4xl mb-4">💚</div>
        <h3 className="text-2xl font-semibold text-emerald-800 dark:text-emerald-300 mb-3">
          Community Powered
        </h3>
        <p className="text-gray-700 dark:text-gray-300 mb-4">
          This database is made possible by the amazing r/SwordAndSupperGame
          community. Every mission you see here has been played, documented,
          and shared by fellow adventurers.
        </p>
        <p className="text-gray-600 dark:text-gray-400 text-sm">
          Thank you to everyone who has contributed missions and helped
          build this resource! 🐸
        </p>
      </div>
    </div>
  );
}
